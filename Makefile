...
PACKAGE := algebra_worksheets
PYTHON_TARGETS := algebra_worksheets tests
PYLINT_MAIN_TARGETS := algebra_worksheets
...
ABOUT_FILE := algebra_worksheets/__about__.py
...
.PHONY: \
	...
	check check-ci \
	worksheet worksheet-source answer-key sample clean-out \
	help

help:
	@echo "Targets:"
	@echo "  worksheet              Generate a worksheet + answer key PDF (scripts/generate_worksheet.sh)"
	@echo "  worksheet-source       Emit LaTeX/HTML source only (scripts/generate_source.sh)"
	@echo "  answer-key             Generate just the compact answer key (scripts/generate_answer_key.sh)"
	@echo "  sample                 Generate a reproducible sample into out/ (scripts/generate_sample.sh)"
	@echo "  clean-out              Remove generated artifacts in out/"
	@echo ""
...
# These targets shell out to ./scripts/*.sh so the generation commands live in
# version-controlled, reusable bash scripts. Override knobs via env vars, e.g.:
#   make worksheet COUNT=24 SEED=42 OUT=out/week1
...
worksheet:
	@COUNT="$(COUNT)" SEED="$(SEED)" OUT="$(OUT)" PAPER="$(PAPER)" \
		$(UV) run bash scripts/generate_worksheet.sh

worksheet-source:
	@COUNT="$(COUNT)" SEED="$(SEED)" OUT="$(OUT)" PAPER="$(PAPER)" \
		$(UV) run bash scripts/generate_source.sh

answer-key:
	@COUNT="$(COUNT)" SEED="$(SEED)" OUT="$(OUT)" PAPER="$(PAPER)" \
		$(UV) run bash scripts/generate_answer_key.sh

sample:
	@$(UV) run bash scripts/generate_sample.sh
