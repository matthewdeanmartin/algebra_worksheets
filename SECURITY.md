# Security Policy for `algebra_worksheets`
...
1. **GitHub private vulnerability reporting** —
   [submit a confidential report](https://github.com/matthewdeanmartin/algebra_worksheets/security/advisories/new)
...
For maximum isolation, run `algebra_worksheets` inside a container so it only has access to what you
explicitly mount.
...
RUN pip install --no-cache-dir algebra_worksheets

ENTRYPOINT ["algebra_worksheets"]
CMD ["--help"]
```
...
```bash
docker build -t algebra_worksheets .

# Mount the current directory into /data and pass it as an argument
docker run --rm -v "$(pwd):/data" algebra_worksheets /data
```
